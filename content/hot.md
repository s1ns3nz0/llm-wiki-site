...
2026-07-02 (later): **PyTorchKR news → wiki ingest pipeline tested end-to-end (manual leg)**. Built GitHub Actions automation for daily PyTorchKR (discuss.pytorch.kr/c/news/14) monitoring: `.github/workflows/pytorch-news-notify.yml` (cron, diffs against `.vault-meta/pytorch-news-seen.json`, sends Telegram via `scripts/pytorch-news-notify.py`) confirmed working after fixing a missing `TELEGRAM_CHAT_ID` secret and a fork-schedule-disabled gotcha (GitHub disables `schedule` triggers on forks by default; fixed via manual "Enable workflow" + `workflow_dispatch` test). Dropped the on-demand `/news` Telegram-bot-poll feature per user call (removed `scripts/pytorch-news-bot-poll.py` + its workflow) in favor of testing the manual pick-and-ingest path instead: user selected article #1 from a notified batch ("DevSpace" self-hosted MCP server article), Claude fetched + summarized it, filed `.raw/pytorch-devspace-mcp.md` + `wiki/sources/DevSpace-ChatGPT-Local-MCP.md` + `wiki/concepts/DevSpace.md`, updated both `_index.md` files and this log. Also stood up Quartz + GitHub Pages publishing: `s1ns3nz0/llm-wiki-site` (Quartz 5, obsidian template) deploys via its own `deploy.yml`; `.github/workflows/sync-wiki-to-quartz.yml` in this repo mirrors `wiki/` into that site's `content/` on push (needs `QUARTZ_SITE_REPO` var + `QUARTZ_SITE_PAT` secret, both registered). **Next step**: commit + push this ingest test (`.raw/pytorch-devspace-mcp.md`, `wiki/sources/DevSpace-ChatGPT-Local-MCP.md`, `wiki/concepts/DevSpace.md`, both `_index.md` files, `wiki/log.md`, this hot.md) so the Quartz sync workflow picks it up; also revert `pytorch-news-notify.yml` cron off its temporary 19:40 KST test time to the intended daily schedule.

2026-07-02: **UAV-AI-SOC 시스템디자인 series ingested (17 docs, Korean)**. User imported existing knowledge from `~/Documents/Obsidian Vault/UAV-AI-SOC/시스템디자인/` — copied to `.raw/UAV-AI-SOC/시스템디자인/` (immutable), then batch-ingested via 3 parallel wiki-ingest waves (6+6+5 agents). Series index: `wiki/references/UAV-AI-SOC-시스템디자인.md` (17/17 complete, `series_complete: true`). Every doc has its `wiki/sources/UAV-AI-SOC-시스템디자인-NN.md` page plus 1-6 concept pages. Domain umbrella entity: `wiki/entities/UAV-AI-SOC.md`. New concept pages (~50) span the full stack: **LangGraph orchestration** (`에이전트 오케스트레이션 파이프라인`, `BaseSOCAgent`, `고정 파이프라인 토폴로지`, `SOCState`, `UAV-AI-SOC Core Layer`, `Alert Model`, `SOCPlatformError Exception Hierarchy`, `Settings SecretStr`), **retrieval** (`RAG`, `GraphRAG`, `Knowledge Graph`, `CompositeRetriever`, `MITRE ATT&CK` entity), **memory + self-improvement** (`Experience Memory`, `Memory Poisoning`, `Self-Improvement Loop`, `ExperienceRecord`, `Asymmetric Trust`, `Provenance Trust`, `OutcomeProbeAgent Pipeline`), **detection-as-code** (`Detection-as-Code`, `Watchlist-as-Detection-Data`, `Auto KQL Rule Agent`, `Threat Landscape Agent`), **external integrations** (`Threat Intel Feed`, `Sandbox Detonation`, `Vulnerability Enrichment`), **environment oracle** (`환경검증 오라클`), **deployment** (`2-Deployment 하이브리드 토폴로지`, `GitOps ArgoCD`, `Prometheus Observability`), **security** (`Defense-in-Depth`, `Attack Surface`, `IoA (Indicator of Attack)`, `Attacker-Centric Defense`), **validation** (`Multi-Judge Ensemble`, `Signal Veto`, `Verdict Aggregation`, `벤치마크 하네스`, `ATLAS 레드팀 벤치마크`, `회귀 게이트`), **quality/audit** (`RAGAS 품질 측정`, `Causal Reasoning Chain`, `Data Lineage Snapshot`, `GNSS Domain Context`). **Known gap**: `flock` unavailable on macOS → `scripts/wiki-lock.sh` fell through in every agent; wave-1 lost 3 manifest/log entries to parallel-write race (backfilled in same session with `backfilled: 2026-07-02` marker). Followup: patch wiki-lock.sh to Python-based cross-process lock, or `brew install util-linux`. **Next step**: user's call — commit the ingest, or explore what to do with the vault now that this domain is loaded.
